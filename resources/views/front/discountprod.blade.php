    ...
    <div class="shop-product breadcrumb1 lg:py-20 md:py-14 py-10">
        <div class="container">
            <div class="flex max-md:flex-wrap max-md:flex-col-reverse gap-y-8">
                <div class="sidebar lg:w-1/4 md:w-1/3 w-full md:pr-12 md:pl-12">
                    <div class="filter-type-block pb-8 border-b border-line">
                        ...
                        <div class="list-type filter-type menu-tab mt-4">
                            @foreach ($categories as $category)
                                <div class="item tab-item flex items-center justify-between cursor-pointer gap-2"
                                    data-item="{{ $category->id }}">
                                    <a href="{{ route('products/discount', ['category_id' => $category->id]) }}" class="flex items-center gap-2">
                                        <img src="{{ asset('images/category/' . ($category->image ?? 'default.png')) }}"
                                            style="width: 30px" class="w-full h-full object-cover" alt="image" />
                                        @if (App::getLocale() == 'ar')
                                            <div
                                                class="type-name text-secondary has-line-before hover:text-black capitalize">
                                                {{ $category->name ?? __('products.unknown_category') }}
                                            </div>
                                        @else
                                            <div
                                                class="type-name text-secondary has-line-before hover:text-black capitalize">
                                                {{ $category->englishname ?? __('products.unknown_category') }}
                                            </div>
                                        @endif
                                    </a>
                                    <div class="text-secondary2 number">{{ $category->products_count ?? 0 }}</div>
                                </div>
                            @endforeach
                            <div class="item tab-item flex items-center justify-between cursor-pointer gap-2">
                                <a href="{{ route('products/discount') }}" class="flex items-center gap-2">
                                    <img src="{{ asset('carall.png') }}" style="width: 30px"
                                        class="w-full h-full object-cover" alt="image" />
                                    <div class="type-name text-secondary has-line-before hover:text-black capitalize">
                                        @lang('products.productsall')</div>

                                </a>
                                <div class="text-secondary2 number">{{ $categoriesproducts }}</div>
                            </div>
                        </div>
                    </div>
                    <div class="filter-size pb-8 border-b border-line mt-8">
                        ...
                        <div class="list-size flex items-center flex-wrap gap-3 gap-y-4 mt-4">
                            @foreach ($sizes as $size)
                                <a href="{{ route('products/discount', ['size_id' => $size->id]) }}"
                                    class="size-item text-button w-[100px] h-[44px] flex items-center justify-center rounded-full border border-line">
                                    {{ $size->name ?? __('products.unknown_size') }}
                                </a>
                            @endforeach
                        </div>
                    </div>
                            ...
                            @foreach ($brands as $brand)
                                <div class="brand-item flex items-center justify-between"
                                    ...
                                    <div class="left flex items-center cursor-pointer">
                                        <a href="{{ route('products/discount', ['brand' => $brand->brandname]) }}"
                                            class="brand-name capitalize pl-2 cursor-pointer hover:underline">
                                            {{ $brand->brandname }}
                                        </a>
                                    </div>
                                </div>
                    ...
                    <div class="filter-color pb-8 border-b border-line mt-8">
                        ...
                        <div class="list-color flex items-center flex-wrap gap-3 gap-y-4 mt-4">
                            @foreach ($colors as $color)
                                <a href="{{ route('products/discount', ['grades_id' => $color->id]) }}"
                                    class="color-item px-3 py-[5px] flex items-center justify-center gap-2 rounded-full border border-line"
                                    data-item="{{ $color->name }}">
                                    <div class="color"
                                        style="background-color: {{ $color->name }}; width: 20px; height: 20px; border-radius: 50%;">
                                    </div>
                                    <div class="caption1 capitalize" id="color-name-{{ $loop->index }}">
                ...
                <div class="list-product-block style-grid lg:w-3/4 md:w-2/3 w-full md:pl-3 lg:ml-6 md:ml-4 ml-3">
                    <div class="filter-heading flex items-center justify-between gap-5 flex-wrap">
                        <div class="sort-product right flex items-center gap-3">
                            <label for="select-filter" class="caption1 capitalize">{{ __('products.sort_by') }}</label>
                            <div class="select-block relative">
                                <select id="select-filter" name="sort"
                                    class="caption1 py-2 pl-3 md:pr-20 pr-10 rounded-lg border border-line"
                                    onchange="location = this.value;">
                                    <option value="{{ route('products/discount') }}" {{ request('sort') == null ? 'selected' : '' }}>
                                        {{ __('products.sort_by') }}
                                    </option>
                                    <option value="{{ route('products/discount', ['sort' => 'priceHighToLow']) }}"
                                        {{ request('sort') == 'priceHighToLow' ? 'selected' : '' }}>
                                        {{ __('products.price_high_to_low') }}
                                    </option>
                                    <option value="{{ route('products/discount', ['sort' => 'priceLowToHigh']) }}"
                                        {{ request('sort') == 'priceLowToHigh' ? 'selected' : '' }}>
                                        {{ __('products.price_low_to_high') }}
                                    </option>
                                </select>
                            ...
                            <a href="{{ route('product/info', encrypt($product->id)) }}"
                                ...
                                <div class="product-infor text-center mt-4">
                                    ...
                                    <div class="mt-3 text-sm text-gray-600">
                                        {{-- <strong>{{ __('products.size') }}:</strong> --}}
                                    ...
                                    <div class="mt-2">
                                        {{-- <strong class="text-sm text-gray-600">{{ __('products.grades') }}:</strong> --}}
