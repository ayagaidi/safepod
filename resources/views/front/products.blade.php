                                ...
                                <div class="list-type filter-type menu-tab mt-4">
                                    @foreach ($categories as $category)
                                        <div class="item tab-item flex items-center justify-between cursor-pointer gap-2 py-2" data-item="{{ $category->id }}">
                                            <a href="{{ route('all_products', ['category_id' => $category->id]) }}" class="flex items-center gap-3">
                                                <span class="thumb-ico">
                                                    <img src="{{ asset('images/category/' . ($category->image ?? 'default.png')) }}" alt="image">
                                                </span>
                                                @if (App::getLocale() == 'ar')
                                    ...
                                    <div class="item tab-item flex items-center justify-between cursor-pointer gap-2 py-2">
                                        <a href="{{ route('all_products') }}" class="flex items-center gap-3">
                                            <span class="thumb-ico"><img src="{{ asset('carall.png') }}" alt=""></span>
                            ...
                            <section class="kid-card mt-6">
                                ...
                                <div class="list-size flex flex-wrap gap-2 mt-4">
                                    @foreach ($sizes as $size)
                                        <a href="{{ route('all_products', ['size_id' => $size->id]) }}" class="kid-chip">{{ $size->name ?? __('products.unknown_size') }}</a>
                                    @endforeach
                                </div>
                            </section>
                                ...
                                <div class="list-brand mt-3 space-y-2">
                                    @foreach ($brands as $brand)
                                        <div class="brand-item flex items-center justify-between" data-item="{{ $brand->brandname }}">
                                            <a href="{{ route('all_products', ['brand' => $brand->brandname]) }}" class="brand-link">{{ $brand->brandname }}</a>
                                        </div>
                                    @endforeach
                                </div>
                            </section>
                            ...
                            <section class="kid-card mt-6">
                                ...
                                <div class="list-color flex flex-wrap gap-2 mt-3">
                                    @foreach ($colors as $color)
                                        <a href="{{ route('all_products', ['grades_id' => $color->id]) }}" class="kid-color" data-item="{{ $color->name }}">
                                ...
                                <div class="sort-product flex items-center gap-2">
                                    ...
                                    <div class="select-block relative">
                                        <select id="select-filter" name="sort" class="kid-select" onchange="location = this.value;">
                                            <option value="{{ route('all_products') }}" {{ request('sort') == null ? 'selected' : '' }}>{{ __('products.sort_by') }}</option>
                                            <option value="{{ route('all_products', ['sort' => 'priceHighToLow']) }}" {{ request('sort') == 'priceHighToLow' ? 'selected' : '' }}>{{ __('products.price_high_to_low') }}</option>
                                            <option value="{{ route('all_products', ['sort' => 'priceLowToHigh']) }}" {{ request('sort') == 'priceLowToHigh' ? 'selected' : '' }}>{{ __('products.price_low_to_high') }}</option>
                                        </select>
                                    </div>
                                </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // لايت بوكس بسيط للصورة (اختياري، نفس فكرتك)
        document.querySelectorAll('.product-image').forEach(img=>{
            img.addEventListener('click', e=>{
                e.preventDefault();
                const ov = document.createElement('div');
                ov.className = 'image-overlay';
                ov.innerHTML = `<div class="overlay-content"><button class="close-overlay" aria-label="Close">&times;</button><img src="${img.src}" class="enlarged-image" alt=""></div>`;
