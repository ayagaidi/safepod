            ...
            <div class="container lg:pt-[124px] pt-24 pb-10 relative">
                <div class="main-content w-full h-full flex flex-col items-center justify-center relative z-[1]">
                    <div class="text-content">
                        <div class="heading2 text-center">{{ __('products.categorypro') }} @if (App::getLocale() == 'ar')
                                :{{ $categoriess->name ?? __('products.unknown_category') }}
                            @else
                                : {{ $categoriess->englishname ?? __('products.unknown_category') }}
                            @endif
                        </div>
                        <div class="link flex items-center justify-center gap-1 caption1 mt-3">
                            <a href="{{ route('/') }}" class="hover:underline">{{ __('products.home') }}</a>
                            <i class="ph ph-caret-right text-sm text-secondary2"></i>
                            <div class="text-secondary2 capitalize">{{ __('products.categorypro') }}  @if (App::getLocale() == 'ar')
                                <i class="ph ph-caret-right text-sm text-secondary2"></i>
                                {{
                                    $categoriess->name ?? __('products.unknown_category') }}
                            @else
                    ...
                    <div class="filter-size pb-8 border-b border-line mt-8">
                        ...
                        <div class="list-size flex items-center flex-wrap gap-3 gap-y-4 mt-4">
                            @foreach ($sizes as $size)
                                <a href="{{ route('product/category', ['id' => $size->id, 'size_id' => $size->id]) }}"
                            ...
                            @foreach ($brands as $brand)
                                <div class="brand-item flex items-center justify-between"
                                    ...
                                    <div class="left flex items-center cursor-pointer">
                                        <a href="{{ route('product/category', ['id' => $id, 'brand' => $brand->brandname]) }}"
                    ...
                    <div class="filter-color pb-8 border-b border-line mt-8">
                        ...
                        <div class="list-color flex items-center flex-wrap gap-3 gap-y-4 mt-4">
                            @foreach ($colors as $color)
                                <a href="{{ route('product/category', ['id' => $id]) }}"
                        ...
                        <div class="sort-product right flex items-center gap-3">
                            ...
                            <div class="select-block relative">
                                <select id="select-filter" name="sort"
                                    ...
                                    <option value="{{ route('product/category', ['id' => $id]) }}"
                                        {{ request('sort') == null ? 'selected' : '' }}>
                                        {{ __('products.sort_by') }}
                                    </option>
                                    <option
                                        value="{{ route('product/category', ['id' => $id, 'sort' => 'priceHighToLow']) }}"
                                        {{ request('sort') == 'priceHighToLow' ? 'selected' : '' }}>
                                        {{ __('products.price_high_to_low') }}
                                    </option>
                                    <option
                                        value="{{ route('product/category', ['id' => $id, 'sort' => 'priceLowToHigh']) }}"
