...
const updateWishlistIcons = () => {
    ...
    wishlistIcons.forEach((wishlistIcon) => {
        ...
        const productId = productItem ? productItem.getAttribute("data-item") : null;
        ...
        const wishlistIconElement = wishlistIcon.querySelector("i");

        if (isProductInWishlist) {
            wishlistIcon.classList.add("active");
            if (wishlistIconElement) {
                wishlistIconElement.classList.remove("ph");
                wishlistIconElement.classList.add("ph-fill");
            }
        } else {
            wishlistIcon.classList.remove("active");
            if (wishlistIconElement) {
                wishlistIconElement.classList.add("ph");
                wishlistIconElement.classList.remove("ph-fill");
            }
        }
    });
};
...
const openModalCart = () => {
    if (modalCartMain) {
        ...
        console.warn("modalCartMain is not found");
    }
};
...
if (closeCartIcon) {
    ...
    console.warn("closeCartIcon is not found");
}
if (continueCartIcon) {
    continueCartIcon.addEventListener("click", closeModalCart);
} else {
    console.warn("continueCartIcon is not found");
}
...
const handleItemModalCart = () => {
    ...
    if (!listItemCart) {
        console.warn("listItemCart is null");
        return;
    }
...
var swiper2 = new Swiper(".mySwiper2", {
    ...
    on: {
        slideChange: function() {
            ...
            document
                .querySelectorAll(".mySwiper .swiper-slide")[activeIndex].classList.add("swiper-slide-thumb-active");
        },
    },
});
...
const updateCompareIcons = () => {
    ...
    compareIcons.forEach((compareIcon) => {
        const productItem = compareIcon.closest(".product-item");
        const productId = productItem ? productItem.getAttribute("data-item") : null;
